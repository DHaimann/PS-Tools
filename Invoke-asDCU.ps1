...
Function Run-DCU {
    ...
    Param (
        ...
        If (!(Test-Path -LiteralPath 'C:\Program Files (x86)\Dell\CommandUpdate\dcu-cli.exe')) {               
            ...
            $WebProxy = New-Object System.Net.WebProxy("http://proxy.domain.com:proxyport", $true)
            $Credentials = New-Object Net.NetworkCredential("user", "password", "domain")
            $Credentials = $Credentials.GetCredential("http://proxy.domain.com", "proxyport", "KERBEROS");
                ...
                Try {
                    ...
                    If ($Type -eq "bios" -or $Type -eq "all") {
                        ...
                        If ($Recommendation) {
                            ...
                            #$CurrentBIOSVersion = $Recommendation.version
